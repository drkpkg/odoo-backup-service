...
impl BackupManager {
    ...
    pub async fn backup_database(&self, config: &DatabaseConfig) -> Result<String> {
        ...
        // Copy backup to host
        let host_backup_path = self.docker.copy_backup_to_host(
            config, 
            &container_backup_path, 
            &self.host_backup_dir
        ).await?;

        // Cleanup container backup file
        self.docker.cleanup_container_backup(config, &container_backup_path).await?;

        log::info!("Backup completed successfully for {}: {}", config.name, host_backup_path);
        Ok(host_backup_path)
    }

    pub async fn backup_all_databases(&self, configs: &[DatabaseConfig]) -> Result<Vec<(String, String)>> {
    ...
    pub async fn cleanup_old_backups(&self, config: &DatabaseConfig) -> Result<u32> {
        ...
        let entries = fs::read_dir(backup_dir)
            .map_err(|e| BackupError::FileSystem(format!("Failed to read backup directory: {}", e)))?;

        for entry in entries {
            let entry = entry.map_err(|e| BackupError::FileSystem(format!("Failed to read directory entry: {}", e)))?;
            let path = entry.path();
            ...
            if path.is_file() {
                ...
                if filename.contains(&config.database_name) {
                    let metadata = entry.metadata()
                        .map_err(|e| BackupError::FileSystem(format!("Failed to get file metadata: {}", e)))?;
                    
                    let modified_time = metadata.modified()
                        .map_err(|e| BackupError::FileSystem(format!("Failed to get file modification time: {}", e)))?;
                    ...
                    if modified_datetime < cutoff_date {
                        ...
                        fs::remove_file(&path)
                            .map_err(|e| BackupError::FileSystem(format!("Failed to delete old backup: {}", e)))?;
                        deleted_count += 1;
                    }
                }
            }
        ...
        log::info!("Cleaned up {} old backup files for {}", deleted_count, config.name);
        Ok(deleted_count)
    }
    ...
    async fn ensure_backup_directory(&self) -> Result<()> {
        ...
        if !backup_dir.exists() {
            fs::create_dir_all(backup_dir)
                .map_err(|e| BackupError::FileSystem(format!("Failed to create backup directory: {}", e)))?;
